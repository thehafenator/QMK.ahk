...
static ThrowLeft(windowIdentifier := "A", timeout := 5000) {
    ...
    static GestureDR(windowIdentifier := "A") {
        ...
        ; if you want to have certain programs just be hidden instead of closed, edit here
            ; if WinActive("ahk_exe PhoneExperienceHost.exe") ||  WinActive("ahk_class Chrome_WidgetWin_1 ahk_exe msedge.exe") || Winactive("ahk_exe zen.exe") || WinActive("Example Editor ahk_exe example-editor.exe") || WinActive("ahk_class Chrome_WidgetWin_1 ahk_exe Spotify.exe") || WinActive('Phone Link') || WinActive("Checklist:") || WinActive("Task List")
            ; {
            ;     try WinHide("A")
            ;     return true
            ; }
