class QMK {
    ...
    static BufferKeyDown(key) {
        ...
        ; STAGE 1: Traditional modifiers bypass - when pressing a normal modier (such as the left physical control button), these shortcuts work fast and also clear the buffer. Press a traditional modifier if your buffer ever gets stuck
        ...
        ; STAGE 2.5: Check INSTANT combos first (always, even during quiet period)
        for buffKey, buffData in this.keyBuffer {
        ...
        ; STAGE 2.6: Check if primary is in combo repeat mode - allows a combo like QMK.SetupCombo("a", "g", (*) => Send("!{Tab}")) to press the a key first hold it down, then press 'g' repeatedly.
        ...
        ; STAGE 2.65: Check for regular combos BEFORE quiet period buffering
        ...
        ; STAGE 2.7: If in quiet period, ALWAYS buffer (typing takes priority unless intentional chord or registered combo). Useful for typing normal words. For example, the word 'please' often has keyboard rolls and homerow keys that are modifiers. This checks that if we are typing, we mark those as tap
        ...
        ; STAGE 3: Check active homerow modifiers - Allows us to press multiple keys that are modifiers and 'stack' them instantly for quick sending other keys
        activeModifierCount := 0
        for modKey, _ in this.activeModifiers {
            activeModifierCount++
        }

        ; STAGE 3.5: Check for instant combos with any unreleased key
        unreleasedCount := 0
        for buffKey, buffData in this.keyBuffer {
            if (!buffData.isReleased) {
                unreleasedCount++
        ...
        ; STAGE 3.75: Check for multiple unreleased modifiers past threshold ;
        ...
        ; STAGE 4: Active homerow modifiers present
        ...
        ; STAGE 5: Check for held keys past threshold (Fast path for modifiers/combos)
        if (unreleasedModifierCount < 2) {
            for buffKey, buffData in this.keyBuffer {
                if (buffData.isReleased)
                    continue
        ...
        ; STAGE 6: Check for same-modifier keys (before multi-modifier detection)
        if (this.modifierKeys.Has(key)) {
            newModName := this.modifierKeys[key]
        ...
        ; STAGE 7: Check for instant combos with unreleased keys
        for buffKey, buffData in this.keyBuffer {
            if (buffData.isReleased)
                continue
    ...
    static FindMatchingCallback(hotkeyId) {
        ...
        if (!state.hasContextMenu) {
            ...
            state.currentUrl := ""
            ; ; comment these seven lines out if you do not have onwebsite.ahk or UIA (don't need website specific support) and when running you get error messages. It should run without though, just skipping this part 
            if IsSet(On) {
                try {
                    if IsObject(On.LastResult) && On.LastResult.HasProp("url") {
                        state.currentUrl := On.LastResult.url
                    }
                }
            }
        ...
        static MatchesContext(context, contextType, state) {
            ...
            ; comment these two lines out if you do not have onwebsite.ahk or UIA (don't need website specific support) and when running you get error messages. It should run without though, just skipping this part 
