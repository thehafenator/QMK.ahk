...
^!F11:: QMK.SaveTrainingData()
^!F12:: QMK.SaveTrainingData()
; ^!F2:: QMK.Toggle_KernelInjectionIni()
...
class QMK {
    ...
    static _coreLoadedFromFile := true
...
static __New() {
    ...
    trainingDll := "C:\Users\" . A_UserName . "\OneDrive\Documents\AutoHotkey\lib\Libraries\QMKInterception\QMKTrainingData.dll"
    optimizedDll := "C:\Users\" . A_UserName . "\OneDrive\Documents\AutoHotkey\lib\Libraries\QMKInterception\QMKCorePGOOptimized.dll"
    trainingDataDir := "C:\Users\" . A_UserName . "\OneDrive\Documents\AutoHotkey\lib\Libraries\QMKInterception\QMKTrainingData"
    DirCreate(trainingDataDir)
    EnvSet("LLVM_PROFILE_FILE", trainingDataDir . "\qmk_pgo_current.profraw")
    dllToLoad := QMK.userconfig.useTrainingDll ? trainingDll : optimizedDll
    QMK.hModule := DllCall("kernel32\LoadLibraryW", "Str", dllToLoad, "Ptr")
    if !QMK.hModule {
        MsgBox("Failed to load QMKCore DLL from:`n" . dllToLoad)
        ExitApp()
    }
    ; ToolTip(QMK.userconfig.useTrainingDll ? "QMK PGO trainer loaded from DLL file" : "QMK PGO optimized DLL loaded from file")
...
DllCall(QMK.Proc("QMK_SetAHKThreadId"), "UInt", DllCall("kernel32\GetCurrentThreadId", "UInt"))
    ...
    static Proc(funcName) {
        ...
        addr := DllCall("kernel32\GetProcAddress", "Ptr", QMK.hModule, "AStr", funcName, "Ptr")
        if !addr
    ...
    ;     profileDir := "C:\Users\s262925\OneDrive\Documents\AutoHotkey\lib\Libraries\QMKInterception\QMKTrainingData"
    ;     DirCreate(profileDir)
    ;     currentProfilePath := profileDir . "\qmk_pgo_current.profraw"
    ;     profilePath := profileDir . "\qmk_pgo_" . stamp . ".profraw"
    ;     try FileDelete(currentProfilePath)
    ;     addr := DllCall("kernel32\GetProcAddress", "Ptr", QMK.hModule, "AStr", "QMK_WriteLLVMProfile", "Ptr")
    ;     if !addr {
    ;         ToolTip("PGO export missing: rebuild instrumented DLL")
    ;         SetTimer(() => ToolTip(), -2500)
    ;         return
    ;     }
    ;     DllCall("kernel32\SetCurrentDirectoryW", "Str", A_ScriptDir, "Int")
    ;     try result := DllCall(addr, "Int")
    ;     catch as err {
    ;         ToolTip("PGO save failed: " . err.Message)
    ;         SetTimer(() => ToolTip(), -2500)
    ;         return
    ;     }
    ;     if (result == 0 && FileExist(currentProfilePath)) {
    ;         try FileMove(currentProfilePath, profilePath, true)
    ;         catch as err {
    ;             ToolTip("PGO rename failed: " . err.Message . "`n" . currentProfilePath)
    ;             SetTimer(() => ToolTip(), -3500)
    ;             return
    ;         }
    ;         ToolTip("PGO training saved: " . FileGetSize(profilePath) . " bytes`n" . profilePath)
    ;     } else {
    ;         ToolTip("PGO save returned " . result . "; no file at`n" . currentProfilePath)
    ;     }
    ;     SetTimer(() => ToolTip(), -2500)
    ; }
    ; this one worked chatgpt
; static SaveTrainingData() {
;     profileDir := "C:\Users\s262925\OneDrive\Documents\AutoHotkey\lib\Libraries\QMKInterception\QMKTrainingData"
;     DirCreate(profileDir)
...
class QMKUserConfig {
    ...
    static AddBoolRow(g, def) {
        ...
        hint := g.AddText("xm y+3 w" . this.INNER_W . " c667085", def.rangeText . "  â€”  " . def.description)
    ...
    static GetUserConfigSchema() {
        return [
            ...
            { key: "useTrainingDll", group: "General", type: "bool", label: "Use Training DLL", default: true
            , rangeText: "true / false", description: "When on, loads QMKTrainingData.dll for PGO data collection. When off, loads QMKCorePGOOptimized.dll. Requires script reload to take effect." },
