...
class QMK {
        ...
        static userconfig := {
            applyUserConfig: true,              ; (true/false, default false) set true to apply all of these timing settings on load. Turn off to compare to default settings. Your specific holds, modifiers, combos, etc., will not be affected
...
static __New() {
    ...
    QMK.hModule := DllCall("LoadLibrary", "Str", libDir "\QMKCore.dll", "Ptr")
    if !QMK.hModule {
        MsgBox("Failed to load QMKCore.dll from: " . libDir . "\QMKCore.dll")
        ExitApp()
    }
    ; the DLL was built with the interception.dll embedded, so no need to load it here. If you need to compiled to a different architechture, you will need to use the correct version of the dll, place it in the same folder, then build the zig dll
    ; QMK.hInterception := DllCall("LoadLibrary", "Str", libDir "\interception.dll", "Ptr")
    ; if !QMK.hInterception {
    ;     MsgBox("Failed to load interception.dll from: " . libDir . "\interception.dll")
    ;     ExitApp()
    ; }
    ...
    DllCall(QMK.Proc("QMK_SetInterceptionCallbacks"),
        "Ptr", DllCall("GetProcAddress", "Ptr", QMK.hInterception, "AStr", "interception_create_context", "Ptr"),
        "Ptr", DllCall("GetProcAddress", "Ptr", QMK.hInterception, "AStr", "interception_destroy_context", "Ptr"),
        "Ptr", DllCall("GetProcAddress", "Ptr", QMK.hInterception, "AStr", "interception_send", "Ptr"),
        "Ptr", DllCall("GetProcAddress", "Ptr", QMK.hInterception, "AStr", "interception_is_keyboard", "Ptr"))
    ...
    static Proc(funcName) {
        ...
        addr := DllCall("GetProcAddress", "Ptr", QMK.hModule, "AStr", funcName, "Ptr")
        if !addr
